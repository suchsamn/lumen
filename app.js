...
async function addNote() {
  ...
  // 1) Criar a nota imediatamente com as imagens em base64 (UI rápida + fallback offline).
  const note = {
    id: noteId,
  ...
  const imagesToUpload = [...pendingNoteImages];
  pendingNoteImages = [];
  renderNoteImagesPreview();
  renderNotes(); renderDashboard();
  toast('Nota salva!');

  // 2) Em segundo plano: fazer upload das imagens para o Storage e substituir
  //    o base64 pela URL pública. Só assim as imagens vão para a nuvem e
  //    sobrevivem ao logout / mudança de dispositivo.
  if (imagesToUpload.length && window._firebaseStorage && window._currentUid) {
    for (let i = 0; i < imagesToUpload.length; i++) {
      const src = imagesToUpload[i];
      if (typeof src !== 'string' || !src.startsWith('data:')) continue;
      const isGif = src.startsWith('data:image/gif');
      const ext = isGif ? 'gif' : 'jpg';
      const url = await _uploadToStorage(`notes/${noteId}/img-${i}-${Date.now()}.${ext}`, src);
      if (url) {
        const n = state.notes.find(x => x.id === noteId);
        if (n && n.images && n.images[i] === src) {
          n.images[i] = url;
          save();
          renderNotes();
        }
...
function deleteNote(id) {
  ...
  // Apagar imagens do Storage (apenas as que já são URLs do Firebase)
  if (note && Array.isArray(note.images)) {
    note.images.forEach(src => {
      if (typeof src === 'string' && src.includes('/o/users%2F')) {
        try {
          const match = src.match(/\/o\/(.+?)\?/);
          if (match) {
            const fullPath = decodeURIComponent(match[1]);
            const userPrefix = `users/${window._currentUid}/`;
            if (fullPath.startsWith(userPrefix)) {
              _deleteFromStorage(fullPath.slice(userPrefix.length));
            }
          }
        } catch (e) { /* ignorar */ }
      }
